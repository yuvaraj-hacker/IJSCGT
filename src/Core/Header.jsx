      ...
      <section className={`bg-primary border-[#002c2a] border  ${menuOpen ? " block" : " md:block hidden"}`}>
        <header className="max-w-[90rem] mx-auto px-5 bg-primary ">
          <div className="flex justify-center md:justify-between flex-wrap items-center py-1">
            <nav ref={dropdownRef}>
              <div className={`space-x-7 py-4 md:block max-w-[65rem] mx-auto relative  ${menuOpen ? "flex flex-col space-y-4" : " "}`}>
                {navLinks.map((link) => (
                  <div
                    key={link.to}
                    ...
                    <Link to={link.to}
                      className={` border-[#002c2a] text-white     flex  justify-center  transition-all duration-300 ease-in-out  relative    lg:text-base md:text-sm  items-center    gap-2 ${isActive(link.to) || hoveredCategory === link.label ? 'md:bg-[#002c2a] text-[#002c2a] md:text-white' : ' '} `}
                      onClick={(e) => {
                        if (link.dropdown) {
                          e.preventDefault();
                        ...
                        <div className="md:absolute left-0   top-full w-[200px]  md:bg-[#AECC53]  transition-all duration-300 ease-in-out opacity-100 scale-y-100 origin-top  grid md:grid-cols-1  rounded-xl   z-10">
                          {link.dropdown.map((dropdownlink) => (
                            <Link
                              ...
                              <p className="hover:border-l-4 border-l-[#002C2A] px-5 text-center">
                                {dropdownlink.label}
                              </p>
                            </Link>
                          ))}
            ...
            <a href="https://ijscgt.com/ijscgt/index.php/ijscgt/about/submissions" target="_blank" rel="noopener noreferrer" className="w-fit md:block hidden" >
              <button className=" bg-[#AECC53] p-3 rounded-full text-[#002C2A] font-bold cursor-pointer">
